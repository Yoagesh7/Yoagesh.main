...
function renderChatHistory() {
    ...
    const welcome = document.getElementById('chat-welcome-msg')?.textContent || "Hello! I'm your PAI mentor. Let's conquer your goals today. What's on your mind?";

    if (!messages.length) {
        log.innerHTML = `
            <div class="msg-wrap-ai">
                <div class="msg-ai" id="chat-welcome-msg">${escapeHtml(welcome)}</div>
                <div class="msg-time">AI MENTOR • NOW</div>
            </div>`;
        return;
    }
    ...
    messages.forEach(msg => {
        const when = new Date(msg.time || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        if (msg.type === 'user') {
            const wrap = document.createElement('div');
            wrap.className = 'msg-wrap-user';
            wrap.innerHTML = `
                <div class="msg-user">${escapeHtml(msg.text)}</div>
                <div class="msg-time">YOU • ${when}</div>
            `;
            log.appendChild(wrap);
        } else {
            const wrap = document.createElement('div');
            wrap.className = 'msg-wrap-ai';
            wrap.innerHTML = `
                <div class="msg-ai">${escapeHtml(msg.text)}</div>
                <div class="msg-time">AI MENTOR • ${when}</div>
            `;
            log.appendChild(wrap);
        }
    });
